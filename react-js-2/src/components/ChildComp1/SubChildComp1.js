...
const SubChildComp1 = ({title,color}) =>{
    // console.log(props);
    return(
        <div className="SubChildComp1">
            <h2>{title}</h2>
            {color && <p style={{color:"red"}}>Color is getting changed to here{color}</p>}
            {color && <p style={{color:"green"}}>{color}it displayedonly in this GrandChild (here we passed the other props attribute as color it is displayed only on the accessed child in Parent i.e..(ChildComp1))</p>}
